...
fn new() {
    ...
    CreateChunkIndex::new(&chunk_index, &path, &vec!())
...
fn build() {
    ...
    let chunk_index = test_data::prepare_chunk_index(fnname);
    let path = test_data::prepare_fs_structure(fnname);
    CreateChunkIndex::new(&chunk_index, &path, &vec!()).expect("Could not create chunk index builder");
...
fn exclude_glob() {
    ...
    CreateChunkIndex::new(&chunk_index, &path, &exclude).expect("Could not create chunk index builder");
